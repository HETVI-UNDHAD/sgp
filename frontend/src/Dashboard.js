  ...
  return (
    <>
      <div className="db-wrap">
        ...
        <aside className="db-sidebar">
          ...
          <div className="db-sidebar-search-wrap">
            <span className="db-sidebar-search-icon">🔍</span>
            <input
            ...
            />
            {sidebarSearch && (
              <button className="db-sidebar-search-clear" onClick={() => setSidebarSearch("")}>✕</button>
            )}
          </div>
          <div className="db-sidebar-groups">
            {groups.filter(g => g.name.toLowerCase().includes(sidebarSearch.toLowerCase())).length === 0
              ? <p className="db-empty-groups">{sidebarSearch ? "No groups found" : "No groups yet"}</p>
              : groups
      ...
      <style>{`
        ...
        .db-sidebar-search-wrap { position:relative; display:flex; align-items:center; margin:0 4px 10px; }
        .db-sidebar-search-icon { position:absolute; left:10px; font-size:12px; pointer-events:none; opacity:0.5; }
        .db-sidebar-search { width:100%; padding:8px 32px 8px 30px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08); border-radius:10px; color:var(--text); font-size:12px; outline:none; transition:all .2s; }
        .db-sidebar-search::placeholder { color:var(--text-muted); }
        .db-sidebar-search:focus { border-color:rgba(124,58,237,0.5); background:rgba(124,58,237,0.06); box-shadow:0 0 0 3px rgba(124,58,237,0.1); }
        .db-sidebar-search-clear { position:absolute; right:8px; background:none; border:none; color:var(--text-muted); font-size:11px; cursor:pointer; padding:2px 4px; border-radius:4px; line-height:1; transition:color .2s; }
        .db-sidebar-search-clear:hover { color:var(--text); }
